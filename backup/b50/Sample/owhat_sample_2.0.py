"""
    Version 2.0.
    Update items:
        (1) Correct function update_odd_owhat().
            Correction is made to support updating start time and end time of given projects.
        (2) Add new function owhat_project_starttime_endtime().
            This function is used to calculate start time and end time of projects.
            Correspondingly, function update_owhat() is updated to support adding new projects
            or updating already-existed projects with start time and end time.
...
SAMPLING_DELAY = 20     # Delay before program starts.
OWHAT_DELAY = 5         # Delay before sampling each fan club.
...
def owhat_project_starttime_endtime(project_id):
    """
        Description:
    ...
    for fan_club_tuple in fan_club_list:
        ...
        if return_dict['result'] == 'success' and return_dict['data'].get('useractivity') is not None:
            ...
            for project in projects:
                ...
                start_time = owhat_project_starttime_endtime(project_id).get("start_time")
                end_time = owhat_project_starttime_endtime(project_id).get("end_time")

                # For new projects, insert them into table project.
                if project_id not in project_list:
                    try:
                        new_data = (project_name, project_id, 'owhat', amount, fanclub_id,
                        ...
                        new_data = (project_id, 'owhat', amount, fanclub_id,
                                    start_time, end_time, datetime.now(), datetime.now())
...
def update_odd_owhat():
    ...
    for field in cursor:
        if field[0] != '' and field[0] is not None:
            odd_project_list.append(field[0])

    for project_id in odd_project_list:
        if project_id != '' and project_id is not None:
            ...
            end_time = owhat_project_starttime_endtime(project_id).get("end_time")
            # Update.
            update_data = (amount, start_time, end_time, datetime.now(), project_id)
            sql = "UPDATE projects SET amount = %s, start_time = %s, end_time = %s, updated_at = %s " \
                  "WHERE project_id = %s and platform = 'owhat'"
            try:
                cursor.execute(sql, update_data)
                conn.commit()
